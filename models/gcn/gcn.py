...
from models.gcn.gcn_utils.gconv import ConvTemporalGraphical
from models.gcn.gcn_utils.graph import Graph
...
class GCN(nn.Module):
    ...
    def __init__(self, in_channels, edge_importance_weighting, **kwargs):
    ...
    def forward(self, feat1, x1, feat2, x2): 
        ...
        if self.vis_embed:
            out1 = torch.cat((feat1.squeeze(), embed1), dim=1)
            out2 = torch.cat((feat2.squeeze(), embed2), dim=1)

            out1 = self.joint_embed(out1)
            out2 = self.joint_embed(out2)
    ...
    def extract_joint_embed(self, feat, x):
        ...
        out = torch.cat((feat, embed), dim=1)
