...
class Panoptic_Hands(DetectionDataset):
    """
    Panoptic Hands Data (hand143_panopticdb) 

    10,372 training samples
    4,445 testing samples
    Source: https://arxiv.org/1704.07809
    """
    def __init__(self, *args, **kwargs):
        super(Panoptic_Hands, self).__init__(*args, **kwargs)
    ...
    def __getitem__(self, idx):
        ...
        occlusions      = np.zeros((self.clip_length, 21), dtype=np.int32)-1 #21 keypoints
        ...
        for frame_ind in range(len(vid_info['frames'])):
            ...
            for obj in frame['objs']:
                ...
                #Hand crop is sc*B where B is max(obj_bbox)
        ...
        vid_data, temp = self.transforms(cv2.copyMakeBorder(input_data[frame_ind], pt, pb, pl, pr, cv2.BORDER_CONSTANT, value=0)[None], {'bbox_data':hand_pts_coords[None, :, mask], 'hand_crop':hand_crop, 'label':labels})
        hand_pts_coords[frame_ind, mask] = temp

        obj_trgt, obj_trgt_wght = self.generate_target(hand_pts_coords[frame_ind])
        ...
        annot_dict['bbox']        = bbox_data
        annot_dict['hand_crop']   = hand_crops
