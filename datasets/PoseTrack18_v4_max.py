...
#Max of object heatmaps on the same frame is the auxiliary input
...
class PoseTrack18_v4_max(DetectionDataset):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(PoseTrack18_v4_max, self).__init__(*args, **kwargs)
        ...
        for idx, item in enumerate(self.samples):
            ...
            #if self.load_type == 'val' and item['base_path'] != '/z/home/natlouis/data/posetrack_data/images/val/001022_mpii_test':
            #    continue 
            ...
            for frm in item['frames']:
                ...
                for obj in objs:
                    ...
                    kpts = np.array(obj['keypoints']).reshape(self.num_keypoints, 3) #17 points (x,y,valid)
                    ...
                    if prev_vid_id != vid_id:
                        ...
                        self.t1_to_t0[frame_id] = prev_frame_id
                    ...
                    trackid = obj['trackid']
                    obj_id  = obj['id']
                    ########Generate normalized keypoints for aux input (edit: not normalized)
    ...
    def __getitem__(self, idx):
        vid_info = copy.deepcopy(self.samples[idx])
        ...
        for frame_ind in range(vid_frames):
            ...
            if self.load_type != 'test': #Test set does not include keypoint annotations
                ...
                obj = frame['objs'][0] #NOTE: Only one object per sample
                trackid   = obj['trackid']
                ...
                #Extract and resize all other valid keypoints on given frame
                #img_objs = self.img_id_to_kpts[frame_id]
                img_objs = self.img_id_to_kpts[self.t1_to_t0[frame_id]]
                for key, obj in img_objs.items():
                    #if key == trackid: #Don't include this current object
                    #    continue 
            ...
            aux_keys = list(img_objs.keys())
            for p_idx, aux_trgt in enumerate(aux_input): 
                plt.subplot(5,5,p_idx+1)
                plt.imshow(temp_data[0].numpy(), interpolation='none', extent=extent)
                plt.imshow(np.max(aux_trgt,axis=0), cmap='jet', alpha=0.5, interpolation='none', extent=extent) 
                plt.title(aux_keys[p_idx])
                plt.colorbar()
        ...
        #aux input will be max of all heatmaps at designated frame
        aux_input = torch.tensor(aux_input)
        #aux_input = torch.cat((aux_input, target))

        aux_input = torch.max(aux_input,dim=0,keepdim=True)[0]
        '''
        plt.figure(2, figsize=(12,8))
        for j_idx in range(self.num_keypoints):
            plt.subplot(5,5,j_idx+1)
