...
class PoseTrack17(DetectionDataset):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(PoseTrack17, self).__init__(*args, **kwargs)
        ...
        self.sc = kwargs['sc'] #parameter search for the scaling. default = 1.15
        ...
        for idx, item in enumerate(self.samples):
            ...
            for frm in item['frames']:
                #if self.load_type == 'train' and not frm['vid_id'] == '010177':
                #    continue 
                ...
                for obj in objs:
                    ...
                    if np.sum(kpts[:,-1]) < 2: #skip if ALL keypoints are occluded or un-annotated or if only 1 keypoint exists
                        continue 
    ...
    def __getitem__(self, idx):
        ...
        for frame_ind in range(vid_frames):
            ...
            if self.load_type != 'test': #Test set does not include keypoint annotations
                ...
                for obj in frame['objs']:
                    ...
                    '''
                    if self.load_type == 'train':
                        if (np.sum(kpts[:,-1]) > num_joints_half_body) and \
                                np.random.rand() < prob_half_body:
                            upper_joints = []
                            lower_joints = []
                            for joint_id in range(self.num_keypoints):
                                if kpts[joint_id,-1] > 0:
                                    if joint_id in self.upper_body_ids:
                                        upper_joints.append(kpts[joint_id,:2])
                                    else:
                                        lower_joints.append(kpts[joint_id,:2])
                            ...
                            #if len(selected_joints) < 2: #this should never be the case
                            #   return None, None
            ...
            ax1.add_patch(rect)
            ax1.scatter(cx,cy)
            temp_kpts = np.array(obj['keypoints']).reshape(self.num_keypoints,3)
            ax1.scatter(temp_kpts[:,0], temp_kpts[:,1])

            plt.subplot(1,2,2)
            extent = np.int(0), np.int(72), np.int(0), np.int(96)

            plt.imshow(temp_data[0].numpy(), interpolation='none', extent=extent)
            plt.imshow(np.sum(target[0], axis=0), cmap='jet', alpha=0.5, interpolation='none', extent=extent)
        ...
        annot_dict['input_crop']  = bbox #duplicate right now, but hard to separate bbox from crop in the affine transform method 
