...
class Manual_Hands(DetectionDataset):
    ...
    def __getitem__(self, idx):
        ...
        heatmap_sum = torch.sum(heatmaps[:-1,:,:], dim=0)
        plt.imshow(heatmap_sum, cmap='jet', interpolation='nearest')
        plt.show()
        '''
        ...
        annot_dict = dict()
        if self.viz:    
            annot_dict['data']        = vid_data #TODO:for visualization purposes only
