...
def posetrack18_fname2id(fname, frameidx):
    ...
    """
    tok = os.path.basename(fname).split("_") #expected: vidname_t0_t1_(opt) 

    if len(tok[-1].zfill(2)) == 2: #frameid includes optional two digit id
        seqidx = int(vid2idx['_'.join(tok[:-3])])
    else:
        seqidx = int(vid2idx['_'.join(tok[:-2])])

    seqtype_idx = seqtype2idx("surg")
